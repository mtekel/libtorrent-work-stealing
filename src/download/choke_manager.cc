...
ChokeManager::balance() {
  ...
  if (adjust > 0) {
    ...
    // call-back.
	  ...
	  //std::cout<< "chokemanager ballance choke" << std::endl << std::flush;	//NEVER happened
...
ChokeManager::cycle(uint32_t quota) {
  ...
  if (m_unchoked.size() > quota){
	  ...
	  //std::cout<< "chokemanager cycle choke" << std::endl << std::flush; //NEVER happened
...
ChokeManager::set_queued(PeerConnectionBase* pc, ChokeManagerNode* base) {
  ...
  if (!is_full() && (m_flags & flag_unchoke_all_new || m_slotCanUnchoke()) &&
      base->time_last_choke() /*+ rak::timer::from_milliseconds(1) <*/ <= cachedTime) {		//there was 10 seconds < instead of milliseconds
...
ChokeManager::set_not_queued(PeerConnectionBase* pc, ChokeManagerNode* base) {
  ...
  if (base->unchoked()) {						//if it was unchoked, it will choke it now?
...
ChokeManager::set_not_snubbed(PeerConnectionBase* pc, ChokeManagerNode* base) {
  ...
  if (!is_full() && (m_flags & flag_unchoke_all_new || m_slotCanUnchoke()) &&
      base->time_last_choke() + rak::timer::from_seconds(1) < cachedTime) {		//there was 10
    m_unchoked.push_back(value_type(pc, 0));
    m_slotConnection(pc, false);
...
ChokeManager::choke_range(iterator first, iterator last, uint32_t max) {
  ...
  for (target_type* itr = target + order_max_size; itr != target; itr--) {
    ...
    iterator split = std::partition(itr->second - (itr - 1)->first, itr->second,
                                    rak::on(rak::mem_ref(&value_type::first), std::bind2nd(m_slotConnection, true)));		//there was true
...
uint32_t weights_upload_unchoke[ChokeManager::order_max_size] = { 1, 3, 9, 0 };	//there was  { 1, 3, 9, 0 }
...
uint32_t weights_download_unchoke[ChokeManager::order_max_size] = { 1, 1, 1, 1 };	//there was... these are 4 bytes that form 32bit value m_unchokeWeight (?)
