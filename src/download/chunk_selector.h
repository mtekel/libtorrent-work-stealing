...
public:
  ...
  void                initialize(Bitfield* bf, ChunkStatistics* cs, STARTstatistics* ST);
  void                cleanup();
  ...
  //for managing START, STARTed is de facto a copy of transfer list, with O(1) access
  std::vector<BlockList*> STARTed;
  std::vector<bool>		  STsolve;


private:
  bool                search_linear(const Bitfield* bf, rak::partial_queue* pq, priority_ranges* ranges, uint32_t first, uint32_t last);
  bool                search_ns_linear(const Bitfield* bf, rak::partial_queue* pq, priority_ranges* ranges, uint32_t first, uint32_t last);
  inline bool         search_linear_range(const Bitfield* bf, rak::partial_queue* pq, uint32_t first, uint32_t last);
  inline bool         search_ns_linear_range(const Bitfield* bf, rak::partial_queue* pq, uint32_t first, uint32_t last);
  inline bool         search_linear_byte(rak::partial_queue* pq, uint32_t index, Bitfield::value_type wanted);
  inline bool         search_ns_linear_byte(rak::partial_queue* pq, uint32_t index, Bitfield::value_type wanted);
