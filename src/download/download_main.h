...
public:
  ...
  STARTstatistics*    START_statistics()                         { return m_STARTstatistics; }
  
  Delegator*          delegator()                                { return &m_delegator; }

  have_queue_type*    have_queue()                               { return &m_haveQueue; }
  START_queue_type*	  START_queue()								 { return &m_STARTQueue;}
  START_queue_type*	  STOP_queue()								 { return &m_STOPQueue;}
  ...
  //void				  slot_disconnect_peer(slot_start_handshake_type s) { m_slotDisconnectPeer = s; }
  ...
  void                remove_peer(const rak::socket_address& sa);
  ...
  //selects my HIGH priority range according to my ID and number of peers sharing the same connection
  void				  select_my_range();

  bool 				  init_hash;
  void				  update_ID(uint32_t ID, uint32_t peerN);

  uint32_t			  myID;			//IDs start with 0!
  uint32_t			  peerNO;
...
private:
  ...
  STARTstatistics*	  m_STARTstatistics;

  Delegator           m_delegator;
  have_queue_type     m_haveQueue;
  START_queue_type	  m_STARTQueue;
  START_queue_type	  m_STOPQueue;
  ...
  //slot_start_handshake_type m_slotDisconnectPeer;
