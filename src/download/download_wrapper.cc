...
DownloadWrapper::initialize(const std::string& hash, const std::string& id) {
  ...
  // Info hash must be calculated from here on.
...
DownloadWrapper::receive_initial_hash() {
  ...
  if (!m_hashChecker->is_checking()) {
    ...
    m_main.chunk_selector()->initialize(m_main.file_list()->mutable_bitfield(), m_main.chunk_statistics(), m_main.START_statistics());
    receive_update_priorities();
    m_main.init_hash = true;
    m_main.select_my_range();
  }
...
DownloadWrapper::receive_hash_done(ChunkHandle handle, const char* hash) {
  ...
  if (m_hashChecker->is_checking()) {
    ...
    if (hash == NULL) {
      ...
      if (std::memcmp(hash, chunk_hash(handle.index()), 20) == 0) {
        m_main.file_list()->mark_completed(handle.index());
        m_main.delegator()->transfer_list()->hash_succeded(handle.index(), handle.chunk());			//here it gets also removed from transfer list
...
//there was this not
        std::cout << rak::timer::current_usec() << " : HASH " << handle.index() << " done" << std::flush;

/*
//remove that piece from START queue
        DownloadMain::START_queue_type::iterator idx = m_main.START_queue()->end();
        if (!m_main.START_queue()->empty()) {	  	idx = std::find_if(m_main.START_queue()->begin(), m_main.START_queue()->end(),
																	rak::equal(handle.index(), rak::mem_ref(&DownloadMain::START_queue_type::value_type::second)));        }

        if ( idx != m_main.START_queue()->end()) {	m_main.START_queue()->erase(idx);
													std::cout << " and erased that piece from START queue." << std::endl;        }
        else { std::cout << " and this piece was NOT! in START queue!" << std::endl; }

        if (m_main.chunk_selector()->STARTed[handle.index()] != NULL){
        	m_main.chunk_selector()->STARTed[handle.index()] = NULL;
        } else {
        	std::cout << " and it was also not in STARTed" << std::endl << std::flush;
        }

        m_main.chunk_selector()->STsolve[handle.index()] = false;
        m_main.STOPed[handle.index()] = false;
*/

//because I have finished downloading this chunk, I want to inform all my peers about it by sending HAVE asap:
...
//   if (ticks % 1 == 0)
  ...
  haveQueue->erase(std::find_if(haveQueue->rbegin(), haveQueue->rend(),
                                rak::less(cachedTime - rak::timer::from_seconds(60),		//there was 600
