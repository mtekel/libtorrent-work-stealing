...
Manager::initialize_download(DownloadWrapper* d) {
  ...
  //d->main()->slot_disconnect_peer(rak::make_mem_fun(m_handshakeManager, &HandshakeManager::disconnect_peer));
...
Manager::receive_tick() {
  ...
  priority_queue_insert(&taskScheduler, &m_taskTick, (cachedTime + rak::timer::from_seconds(3)).round_seconds());	//there was 30
}
