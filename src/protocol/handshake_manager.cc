...
/*
void
HandshakeManager::disconnect_peer(const rak::socket_address& sa, DownloadMain* download) {

//MADE OTHER FUNCTION SOMEWHERE ELSE TO DO THE SAME THING

	std::cout << "connected peers: " << size() << std::endl;

	for (iterator i = begin(); i!=end(); ){
		const rak::socket_address* temp= (*i)->socket_address();
		std::cout << temp->address_str() << ":" <<  temp->port() << std::endl;
		i++;
	}
	std::cout << std::flush;


	iterator itr = std::find_if(base_type::begin(), base_type::end(), std::bind1st(handshake_manager_equal(), &sa));

    if (itr == base_type::end()) {
      std::cout << "HandshakeManager::disconnect_peer(...) could not find handshake of " << sa.address_str() <<":" << sa.port() << std::endl << std::flush;
    } else {
    	std::cout << "HandshakeManager::disconnect_peer(...) removing handshake." << std::endl << std::flush;
    	handshake_manager_delete_handshake(*itr);
        //base_type::erase(itr);		//?? also do this?
    }

    if (!find(sa)) {
          std::cout << "HandshakeManager::disconnect_peer(...) removed handshake." << std::endl << std::flush;
    } else {
    	  std::cout << "HandshakeManager::disconnect_peer(...) couldn't remove handshake." << std::endl << std::flush;
    }
