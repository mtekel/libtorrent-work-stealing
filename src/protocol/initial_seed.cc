...
InitialSeeding::chunk_seen(uint32_t index, PeerConnectionBase* pcb) {
  ...
  if (m_download->chunk_statistics()->complete() > 0)		//there was > 1
    complete(pcb);
...
InitialSeeding::new_peer(PeerConnectionBase* pcb) {
  ...
  //there was this little cheat not:
  //if (m_peerChunks[m_nextChunk]== chunk_unknown) m_peerChunks[m_nextChunk]= chunk_unsent;
...
InitialSeeding::chunk_offer(PeerConnectionBase* pcb, uint32_t chunkDone) {
  ...
  // Otherwise check if we can offer a chunk normally.
    ...
    if (!peer->is_restart()) {		//is blocked = already got a piece
    	std::cout << " !peer->is_restart() " << std::endl << std::flush;
    	clear_peer(peer);
      /*return no_offer;*/}
  ...
  if (m_peerChunks[index] != chunk_unsent) {
    ...
    if (m_peerChunks[index] != chunk_done && (*m_download->chunk_statistics())[index] > 0)		//there was 1
  ...
  if (m_chunksLeft == 1 && valid_peer(m_peerChunks[index])) {			//there was 1 instead of 5
  ...
  if (pcb->bitfield()->get(index)){
	  std::cout << " pcb->bitfield()->get("<<index<<") " << std::endl << std::flush;
...
InitialSeeding::find_next(bool secondary, PeerConnectionBase* pcb) {
  ...
  do {
    ...
    if (m_peerChunks[m_nextChunk] != chunk_done && (*m_download->chunk_statistics())[m_nextChunk] > 0)		//there was > 1
...
InitialSeeding::complete(PeerConnectionBase* pcb) {
  ...
  for (uint32_t i = 0; i < m_download->file_list()->size_chunks(); i++) {
    if /*(true){*/ (m_download->chunk_statistics()->complete() + (*m_download->chunk_statistics())[i] < 1) {			//there was < 2
  ...
  std::cout << "nextchunk is " << m_nextChunk << std::endl <<std::flush;


  if (m_chunksLeft)
    return;

  //m_download->initial_seeding_done(pcb);		//really done? or maybe just reset chunk statistics, set all m_peerChunks to chunk_unsent? or unknown? and continue running?

//THIS breaks it - instead of disconnecting peers, the seeder remains in init seed mode, but still won't sent any chunk out again

  //m_nextChunk = 0;

  if (m_nextChunk == no_offer) m_nextChunk=(int)(0.7 * m_download->file_list()->size_chunks());

  m_chunksLeft= m_download->file_list()->size_chunks();

  std::cout << "about to reset chunk and peerchunk statistics" << std::endl << std::flush;

  m_download->chunk_statistics()->m_complete=0;
  //m_download->chunk_statistics()->m_accounted=0;
  for (uint32_t i = 0; i < m_download->file_list()->size_chunks(); i++) {
	  (*m_download->chunk_statistics())[i]=0;
	  //m_peerChunks[i] =  chunk_unsent;
  }
...
//restart???
  for (PeerList::const_iterator itr = m_download->peer_list()->begin(); itr != m_download->peer_list()->end(); ++itr)
      itr->second->set_flags(PeerInfo::flag_restart);
