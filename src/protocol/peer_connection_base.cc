...
//shouldn't be insert poll after putting it into cancel queue??? and not before??? or rather not? so that I don't queue it again after canceling? Let's try & see...
//CANCEL_SENDING
...
PeerConnectionBase::down_chunk() {
  ...
  if (!m_downChunk.chunk()->is_writable())
    throw internal_error("PeerConnectionBase::down_part() chunk not writable, permission denied");
...
PeerConnectionBase::down_chunk_skip_process(const void* buffer, uint32_t length) {
  ...
  if (!m_downChunk.chunk()->compare_buffer(buffer, transfer->piece().offset() + transfer->position(), compareLength)) {
    ...
    std::cout << "Data does not match what was previously downloaded, chunk " << transfer->piece().index() << ", offset " << transfer->piece().offset() << std::endl << std::flush;
...
PeerConnectionBase::read_request_piece(const Piece& p) {
  ...
  //no return here, instead I DO queue it, even more times - since I can get more cancels!
...
PeerConnectionBase::read_cancel_piece(const Piece& p) {
  ...
  std::cout << "got CANCEL " << p.index() << ", offset " << p.offset() << ", length " << p.length() << std::endl;

  if (itr != m_peerChunks.upload_queue()->end()){
    m_peerChunks.upload_queue()->erase(itr);}
  else {
	  std::cout << "...that block was not in upload queue" << std::endl << std::flush;
  }
}  
...
PeerConnectionBase::try_request_pieces() {
  ...
  //uint32_t pipeSize = download_queue()->calculate_pipe_size(m_peerChunks.download_throttle()->rate()->rate());  		//NON_PATCHED
  ...
  while (download_queue()->queued_size() < pipeSize && m_up->can_write_request()) {
    ...
    //send START here? but then all the conflicts would be HARD? or soft conflict resolve is to send CANCEL to all the peers whom we have sent requests for blocks?
