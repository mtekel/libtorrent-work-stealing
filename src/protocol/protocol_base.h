...
public:
  ...
  void                write_START(uint32_t index);
  void                write_STOP(uint32_t index);
  ...
  static const size_type sizeof_START        = 9;
  static const size_type sizeof_STOP         = 9;
  static const size_type sizeof_have_body    = 4;
  static const size_type sizeof_START_body   = 4;
  static const size_type sizeof_STOP_body    = 4;
  ...
  bool                can_write_START() const                 { return m_buffer.reserved_left() >= sizeof_START; }
  bool                can_write_STOP() const                  { return m_buffer.reserved_left() >= sizeof_STOP; }
  ...
  bool                can_read_START_body() const             { return m_buffer.remaining() >= sizeof_START_body; }
  bool                can_read_STOP_body() const              { return m_buffer.remaining() >= sizeof_STOP_body; }
...
inline void
ProtocolBase::write_START(uint32_t index) {
  m_buffer.write_32(5);
  write_command(START);
  m_buffer.write_32(index);
}
...
inline void
ProtocolBase::write_STOP(uint32_t index) {
  m_buffer.write_32(5);
  write_command(STOP);
  m_buffer.write_32(index);
}
