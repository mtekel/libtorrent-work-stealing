...
RequestList::downloading(const Piece& piece) {
  ...
  if (itr == m_queued.end()) {
	  std::cout << "chunk " << piece.index() << ", offset " << piece.offset() << " not found in the m_queued" << std::flush << std::endl;
    ...
    if (itr == m_canceled.end()) {
      ...
      std::cout << "chunk " << piece.index() << ", offset " << piece.offset() << " not found even in the m_canceled" << std::flush << std::endl;
  ...
  // We received an invalid piece length, probably zero length due to
...
//PATCHED: http://libtorrent.rakshasa.no/attachment/ticket/944/canceled_keep.diff
...
//  }
	...
	if (m_canceled.size() > 1024) {
	 		    ReserveeList::iterator cancel_itr = m_canceled.begin();
	 		    std::advance(cancel_itr, 512);
	 		    std::for_each(m_canceled.begin(), cancel_itr, std::ptr_fun(&Block::release));
	 		    m_canceled.erase(m_canceled.begin(), cancel_itr);
	}
  ...
  while (m_queued.begin() != end) {
    ...
    std::cout << "cancel_range: chunk " << transfer->piece().index() << ", offset " << transfer->piece().offset() << " : ";

    if (transfer->is_valid()) {
      Block::stalled(transfer);
      m_canceled.push_back(transfer);

      std::cout << "pushed back to m_cancelled"<< std::endl << std::flush;
