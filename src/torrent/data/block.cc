...
Block::~Block() {
  ...
  //std::cout << "in block " << m_piece.index() << " destructor" << std::endl << std::flush;
...
Block::erase(BlockTransfer* transfer) {
  ...
  std::cout << "Block::erase(...) transfer already erased." << std::endl << std::flush;
  ...
  if (transfer->is_queued()) {
    ...
    if (itr == m_queued.end()) {
//      throw internal_error("Block::erase(...m_queued) Could not find transfer.");
    	std::cout << "Block::erase(...m_queued) Could not find transfer." << std::endl << std::flush;
    	found=false;

    } else {
    	std::cout << "about to erase block " << transfer->piece().index() << ", offset " << transfer->piece().offset() << " from m_queued of the BLOCK" << std::endl << std::flush;
    	m_queued.erase(itr);
    ...
    if (itr == m_transfers.end()){
      //throw internal_error("Block::erase(...m_transfers) Could not find transfer.");
    	std::cout << "Block::erase(...m_transfers) Could not find transfer." << std::endl << std::flush;found=false;} else {

    // Need to do something different here for now, i think.
    std::cout << "about to erase block " << transfer->piece().index() << ", offset " << transfer->piece().offset() << " from m_transfers of the BLOCK" << std::endl << std::flush;
    m_transfers.erase(itr);}

    if (found && (transfer == m_leader) ) {
...
//    throw internal_error("Block::erase(...) Transfer is finished.");
	  std::cout << "Block::erase(...) Transfer is finished." << std::endl << std::flush;
  }

  if (transfer->peer_info() != NULL)
    transfer->peer_info()->set_transfer_counter(transfer->peer_info()->transfer_counter() - 1);

  std::cout << "about to set_block(NULL) and then delete BlockTransfer*" << std::endl << std::flush;

  if (found) {transfer->set_block(NULL);}
...
Block::completed(BlockTransfer* transfer) {
  ...
  if ((Block::size_type)std::count_if(m_parent->begin(), m_parent->end(), std::mem_fun_ref(&Block::is_finished)) < m_parent->finished())
    throw internal_error("Block::completed(...) Finished blocks too large.");		//I'd rather say too many?
...
Block::transfer_dissimilar(BlockTransfer* transfer) {
  ...
  std::cout << "Block " << transfer->index() << " , offset " << transfer->piece().offset() << " transfer dissimilar called" << std::endl << std::flush;
...
Block::release(BlockTransfer* transfer) {
  if (!transfer->is_valid()) {
    if (transfer->peer_info() != NULL)
      transfer->peer_info()->set_transfer_counter(transfer->peer_info()->transfer_counter() - 1);
    std::cout << "releasing non-valid BlockTransfer, index " << transfer->piece().index() << ", offset " << transfer->piece().offset() << std::endl << std::flush;
    delete transfer;
  } else {
	std::cout << "about to erase BlockTransfer" << std::flush << std::endl;
...
Block::invalidate_transfer(BlockTransfer* transfer) {
  ...
  if (!transfer->is_valid()) {
    if (transfer->peer_info() != NULL)
      transfer->peer_info()->set_transfer_counter(transfer->peer_info()->transfer_counter() - 1);
    std::cout << "invalidating non-valid BlockTransfer, index " << transfer->piece().index() << ", offset " << transfer->piece().offset() << std::endl << std::flush;
    delete transfer;
    ...
    transfer->set_block(NULL);
    std::cout << "about to invalidate BlockTransfer, index " << transfer->piece().index() << ", offset " << transfer->piece().offset() << std::endl << std::flush;
...
void
Block::remove_from_peer(BlockTransfer* transfer) {
	if ( transfer->peer_info()->connection()->download_queue()->m_queued.begin() != transfer->peer_info()->connection()->download_queue()->m_queued.end())  {

		std::deque<BlockTransfer*>::iterator itr = std::find_if(transfer->peer_info()->connection()->download_queue()->m_queued.begin(),
																transfer->peer_info()->connection()->download_queue()->m_queued.end(),
																rak::equal(transfer->piece(), std::mem_fun(&BlockTransfer::piece) ) );
		if (itr != transfer->peer_info()->connection()->download_queue()->m_queued.end()) {transfer->peer_info()->connection()->download_queue()->m_queued.erase(itr);}
		else { std::cout << "!!! remove_from_peer did not find transfer "<< transfer->index() <<" in the peer's queue" << std::endl << std::flush;}
	}
}

void
Block::destroy(BlockTransfer* transfer) {
	if ( transfer != NULL)  {
			delete transfer;
	} else {
			std::cout << "Trying to destroy NULL transfer!" << std::endl << std::flush;
	}
}
