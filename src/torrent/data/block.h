...
//there was const transfer_list_type* queued() const, also for transfers()
  transfer_list_type* queued() 											 { return &m_queued; }
  transfer_list_type* transfers() 										 { return &m_transfers; }
  ...
  void 						remove_from_peer(BlockTransfer* transfer);	//removes the blocktransfer from peers queued queue
  void 						destroy(BlockTransfer* transfer);
