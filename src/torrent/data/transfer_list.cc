...
TransferList::erase(iterator itr) {
  ...
  std::cout << "Transfer list size is " << size() << std::endl;
...
//TODO: convert to use STARTed instead, to avoid find(index) complexity (e.g. O(1) instead of O(n) )
void
TransferList::hash_succeded(uint32_t index, Chunk* chunk) {
  ...
  std::cout << "transfer list: hash_succeeded, about to erase chunk " << index << " from transfer list" << std::endl << std::flush;
  erase(blockListItr);
}
